...
### Changed
- **BREAKING**: Upgrade to Bevy 0.19.0
- Migrated from `EventWriter`/`EventReader` to `MessageWriter`/`MessageReader`
- Migrated to `ChildOf` relationship and `EntityEvent` for `TopicMessage`
- Migrated observer API to `On<Remove, T>` with closure-based triggering
- All event types now implement both `Event` and `Message` traits
